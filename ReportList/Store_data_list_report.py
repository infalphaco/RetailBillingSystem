...
class StoreDataList:
    def __init__(self, root):
        self.root = root
        self.root.title("FIREFIX RETAIL BILLING SYSTEM | Store data list Report")
    ...
    def export_bills_to_excel(self, start_datetime, end_datetime):
        try:
            self.cursor.execute("SELECT * FROM csv_data WHERE timestamp BETWEEN ? AND ?", (start_datetime, end_datetime))
            bills_data = self.cursor.fetchall()

            if not bills_data:
                messagebox.showinfo("No Data", "No Store data found for the selected date and time range.")
                return
            ...
            headers = ["ID", "Item name", "Unit price", "Stock", "Date time"]
