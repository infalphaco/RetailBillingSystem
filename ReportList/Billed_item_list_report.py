...
class BilledItemList:
    def __init__(self, root):
        self.root = root
        self.root.title("FIREFIX RETAIL BILLING SYSTEM | Billed item list Report")
    ...
    def export_bills_to_excel(self, start_datetime, end_datetime):
        try:
            self.cursor.execute("SELECT * FROM bill_items WHERE timestamp BETWEEN ? AND ?", (start_datetime, end_datetime))
            bills_data = self.cursor.fetchall()

            if not bills_data:
                messagebox.showinfo("No Data", "No Biled items found for the selected date and time range.")
                return
            ...
            headers = ["ID", "Bill ID", "Product ID", "quantity", "Date time"]
            ...
            messagebox.showinfo("Success", f"Store data has been exported to {file_path}")
